...
#tuneGrid <- expand.grid(sigma=seq(0, 2, length = 20),C=seq(0, 2, length = 20))
#svmGrid <- data.frame(C=rep(1,10), sigma=1:10/100)
# Fit the model on the training set
set.seed(345) 
model <- train(
  type ~., data = training, method = "svmRadial",
  trControl = ctrl,
  #tuneGrid = svmGrid,
...
plot(model)
model$results
############### choose: sigma=0.01491391, C=1 to get the best ROC
...
set.seed(345)
tuneGrid <- expand.grid(sigma = 0.01491391, C=1)
set.seed(345)
svm_tuned <- train(type~., 
                   data=training, 
                   method='svmRadial', 
...
orig_fit
#orig_fit$finalModel
svm_training_predict_result <-  orig_fit$pred

############ calculate ROC 
l2 = length(svm_training_predict_result$pred)
cvrepeated_time <- 3
l_training <- l2/cvrepeated_time
No1_fold <- svm_training_predict_result[c(1:l_training),]
l_training3 <- l_training*2
l_training2 <- l_training+1
No2_fold <- svm_training_predict_result[c(l_training2:l_training3),]
l_training4 <- l_training3+1
l_training5 <- l_training*3
No3_fold <- svm_training_predict_result[c(l_training4:l_training5),]
...
write.table(test_prediction_result,"test_predict_svm.xls",sep="\t",col.names=NA)
...
write.table(new2,"svm_PBD_predict1.xls",sep="\t",col.names=NA)
...
write.table(new4,"verify_svm_PBD.xls",sep="\t",col.names=NA)
